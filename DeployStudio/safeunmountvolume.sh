#!/bin/sh
## This is a modifiaction of the DeployStudio saveunmountdisk.sh script
## When running the bootcamp_set_name_from_file.sh script we need to unmount the Bootcamp Volume
...
if [ ${#} -lt 1 ]
then
  echo "Usage: ${SCRIPT_NAME} /dev/disk<ID>/s<ID>"
  echo "Example: ${SCRIPT_NAME} /dev/disk0s3"
  exit 1
fi
...
do
  ...
  OUTPUT=`diskutil unmount ${DISKUTIL_OPTS} "${1}" 2>&1`
